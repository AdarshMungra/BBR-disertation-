                      ...
                      const struct rate_sample *rs, struct bbr_context *ctx)
{
    ...
    static const char ack_phase[] = {
        /* bbr_ack_phase strings */
    ...
    snprintf(debugmsg, sizeof(debugmsg) - 1,
             ...
             (rs->lost + rs->delivered) > 0 ? (1000 * rs->lost /
                                               (rs->lost + rs->delivered))
                                            : 0, /* lr: loss rate x1000 */
             (rs->delivered) > 0 ? (1000 * rs->delivered_ce /
                                    (rs->delivered))
                                 : 0,                                     /* er: ECN rate x1000 */
             1000 * bbr->ecn_alpha >> BBR_SCALE,                          /* ea: ECN alpha x1000 */
             bbr->bw_lo == ~0U ? -1 : (s64)bbr_rate_kbps(sk, bbr->bw_lo), /* bwl */
             bbr->inflight_lo,                                            /* il */
             bbr->inflight_hi,                                            /* ih */
                            ...
                            union tcp_cc_info *info)
{
        ...
        ext & (1 << (INET_DIAG_VEGASINFO - 1)))
    {
        ...
        u64 bw_lo = bbr->bw_lo == ~0U ? ~0ULL : bbr_bw_bytes_per_sec(sk, bbr->bw_lo);
